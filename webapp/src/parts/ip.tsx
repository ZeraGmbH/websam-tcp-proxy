...
export const Ip: React.FC<IIpProps> = (props) => {
  ...
  const onChange = React.useCallback(
    (ev: React.ChangeEvent<HTMLInputElement>) =>
      settings.update("proxyIp", ev.currentTarget.value),
    [settings]
  );
  ...
  React.useEffect(() => {
    electronHost.addListener("auto-start", onStart);

    return () => electronHost.removeListener("auto-start", onStart);
  }, [onStart]);
  ...
  return (
    <fieldset className={clsx(styles.ip, props.className)}>
      ...
      <input
        placeholder="(Name oder IP Adresse)"
        type="text"
