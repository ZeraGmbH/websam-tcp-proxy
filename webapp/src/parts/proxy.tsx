...
import { IProxyConfiguration } from '../settings'
...
export const Proxy: React.FC<IProxyProps> = (props) => {
    ...
    const setEndpoint = React.useCallback(
        (ev: React.ChangeEvent<HTMLInputElement>) => onUpdate(index, { ...proxy, endPoint: ev.target.value }),
        [index, onUpdate, proxy]
    )
    ...
    return (
        <div className={clsx(styles.proxy, props.className)}>
            ...
            <input placeholder='(Server:Port)' type='text' value={endPoint} onChange={setEndpoint} />
