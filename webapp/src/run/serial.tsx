...
export const Serial: React.FC<ISerialProps> = (props) => {
  ...
  const [state, setState] = React.useState<[boolean, number] | undefined>(
    undefined
  );
  ...
  React.useEffect(() => {
    ...
    const serial = SerialProxy.create(settings, ports[index]);
    ...
    if (serial) {
      ...
      electronHost.send({
        portName: serial.device,
        type: "port-request",
      } satisfies ISetSerialPortRequest);
    }
